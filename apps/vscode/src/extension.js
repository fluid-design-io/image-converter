 ...
 */
async function convertToAvif(inputPath, shouldCleanup) {
    // If the file is already .avif, skip re-conversion
    if (inputPath.endsWith(".avif")) {
        return inputPath;
    }
    const outputPath = inputPath.replace(/\.\w+$/, ".avif");
    vscode.window.showInformationMessage(`Converting: ${path.basename(inputPath)} → ${path.basename(outputPath)}`);
    try {
        ...
        if (shouldCleanup) {
            try {
                ...
                vscode.window.showInformationMessage(`Cleaned up original file: ${path.basename(inputPath)}`);
            }
            catch (unlinkErr) {
        ...
        throw new Error(`Failed to convert image '${path.basename(inputPath)}' to AVIF. Please check if the file is valid and not corrupted.`);
    }
}
/**
...
async function getImageDimensions(filePath) {
    try {
        ...
        throw new Error(`Failed to read dimensions for '${path.basename(filePath)}'. Is it a valid image?`);
    }
}
/**
...
export function activate(context) {
    ...
    const disposable = vscode.commands.registerCommand("processImage", async () => {
        var _a, _b, _c, _d, _e, _f, _g;
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage("No active editor found. Please open a file to insert an image snippet.");
            return;
        }
        // Grab config
        const config = vscode.workspace.getConfiguration();
        const pathPrefix = (_a = config.get("dynamicImage.defaultPathPrefix")) !== null && _a !== void 0 ? _a : "/public";
        const lightSubfolder = (_b = config.get("dynamicImage.lightSubfolder")) !== null && _b !== void 0 ? _b : "light";
        const darkSubfolder = (_c = config.get("dynamicImage.darkSubfolder")) !== null && _c !== void 0 ? _c : "dark";
        const defaultExt = (_d = config.get("dynamicImage.defaultImageExtension")) !== null && _d !== void 0 ? _d : "png";
        const cleanupOriginal = (_e = config.get("dynamicImage.cleanupOriginal")) !== null && _e !== void 0 ? _e : false;
        ...
        if (!imageName) {
            vscode.window.showWarningMessage("No image name entered; command canceled.");
            return;
        }
        ...
        try {
            ...
            const workspaceFolder = (_g = (_f = vscode.workspace.workspaceFolders) === null || _f === void 0 ? void 0 : _f[0]) === null || _g === void 0 ? void 0 : _g.uri.fsPath;
            if (!workspaceFolder) {
                throw new Error("No workspace folder open. Please open a workspace before using this extension.");
            }
            // Build absolute paths for light and dark images
            const lightInputPath = path.join(workspaceFolder, pathPrefix, lightSubfolder, imageName);
            const darkInputPath = path.join(workspaceFolder, pathPrefix, darkSubfolder, imageName);
            ...
            // Convert both images
            const avifLightPath = await convertToAvif(lightInputPath, cleanupOriginal);
            const avifDarkPath = await convertToAvif(darkInputPath, cleanupOriginal);
...
/>`;
            ...
            vscode.window.showInformationMessage("Dynamic Image snippet inserted successfully!");
        }
        catch (error) {
